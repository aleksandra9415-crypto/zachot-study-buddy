      ...
      <div className="grid grid-cols-[0.8fr_1.4fr_1.4fr] gap-4 items-stretch">
        ...
        <div className="flex flex-col">
          ...
          {comparison.map((row, idx) => (
            <div key={idx} className="min-h-[64px] flex items-center justify-end text-[14px] text-ink-900 font-semibold text-right pr-4">
              {row.label}
            </div>
          ))}
        </div>
        ...
        <div className="bg-white rounded-[24px] px-5 flex flex-col border border-orange-50/50 shadow-sm">
          <div className="min-h-[64px] flex items-center gap-3">
            <div className="w-6 h-6 bg-amber-500 rounded-[6px] flex items-center justify-center text-white font-bold text-sm">
              З
            </div>
            <span className="font-bold text-ink-950 text-[16px]">{BRAND.name}</span>
          </div>
          {comparison.map((row, idx) => (
